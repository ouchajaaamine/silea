...
import com.example.silea.service.*;
...
public class DashboardController {
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved dashboard statistics",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"orders\": {\"totalOrders\": 100, \"pendingOrders\": 10, \"completedOrders\": 80, \"cancelledOrders\": 10},\"customers\": {\"totalCustomers\": 50, \"activeCustomers\": 40, \"inactiveCustomers\": 10},\"products\": {\"totalProducts\": 200, \"activeProducts\": 180, \"inactiveProducts\": 20, \"lowStockProducts\": 5},\"tracking\": {\"totalTrackings\": 90, \"inTransit\": 30, \"delivered\": 50, \"pending\": 10}}"))),
        @ApiResponse(responseCode = "500", description = "Error retrieving dashboard statistics",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"message\": \"Error retrieving dashboard statistics: Internal Server Error\"}")))
    })
    ...
    public ResponseEntity<?> getRecentOrders(@RequestParam(defaultValue = "10") int limit) {
        try {
            var orders = orderService.getRecentOrdersForDashboard(limit);
            
            // Build simplified response for dashboard
            var orderList = orders.stream().map(order -> {
                Map<String, Object> orderMap = new HashMap<>();
                orderMap.put("id", order.getId());
                orderMap.put("orderNumber", order.getOrderNumber());
                orderMap.put("customerName", order.getCustomer().getName());
                orderMap.put("customerEmail", order.getCustomer().getEmail());
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved delivery performance metrics",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"averageDeliveryTime\": 24.5, \"delayedOrdersCount\": 5}"))),
        @ApiResponse(responseCode = "500", description = "Error retrieving delivery performance metrics",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"message\": \"Error retrieving delivery performance: Internal Server Error\"}")))
    })
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved inventory alerts",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"unavailableProducts\": 2, \"totalProducts\": 200}"))),
        @ApiResponse(responseCode = "500", description = "Error retrieving inventory alerts",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"message\": \"Error retrieving inventory alerts: Internal Server Error\"}")))
    })
    public ResponseEntity<?> getInventoryAlerts() {
        try {
            Map<String, Object> alerts = new HashMap<>();
            alerts.put("unavailableProducts", productService.getProductStatistics().getUnavailableProducts());
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved dashboard alerts",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"unavailableProductsCount\": 2, \"pendingOrdersCount\": 3, \"delayedDeliveriesCount\": 2}"))),
        @ApiResponse(responseCode = "500", description = "Error retrieving dashboard alerts",
            content = @Content(mediaType = "application/json", examples = @ExampleObject(value = "{\"message\": \"Error retrieving dashboard alerts: Internal Server Error\"}")))
    })
    public ResponseEntity<?> getDashboardAlerts() {
        try {
            ...
            alerts.put("unavailableProductsCount", productService.getProductStatistics().getUnavailableProducts());
